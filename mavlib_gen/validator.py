...
class MavlinkXmlValidator(object):
    ...
    def __report_schama_validation_error(
        ...
        log.error(reason)
        if xmlschema.helpers.is_etree_element(xsve.elem):
            log.error("\n{}".format(xmlschema.etree_tostring(xsve.elem, xsve.namespaces, "", 5)))
    ...
    def expand_includes(
        self, validated_xmls: Dict[str, MavlinkXmlFile]
    ) -> Tuple[Dict[str, MavlinkXmlFile], netx.DiGraph]:
        """
                ...
                if len(validated_xmls[cur_dialect].xml.includes) == 0:
                    ...
                    for include_path in validated_xmls[cur_dialect].xml.includes:
                        ...
                        if not os.path.isfile(abs_include_path):
                            ...
                            log.debug(
                                "  Expected absolute path that doesnt exist: '{}'".format(
                                    abs_include_path
                                )
                            )
                            return None
                        ...
                        if uniqueness_result == MSG_DEF_DUPLICATE_ERR:
                            log.error(
                                "Second non-identical include path originated from '{}' include"
                            ...
                            raise ValueError(
                                "Unknown result {} from validator.is_msg_def_unique".format(
                                    uniqueness_result
                                )
                            )
    ...
    def generate_dependency_list(
        self, validated_xmls: Dict[str, MavlinkXmlFile], include_graph: netx.DiGraph
    ) -> None:
        """
    ...
    def validate(self, xmls: List[str]) -> Dict[str, MavlinkXmlFile]:
        ...
        :return: Dictionary of XML filename -> MavlinkXmlFile object (deserialized version of XML
            file)
        """
        ...
        for xml_filename in xmls:
            ...
            if uniqueness_result == MSG_DEF_UNIQUE:
                ...
                raise ValueError(
                    "Unknown result {} from validator.is_msg_def_unique".format(uniqueness_result)
                )
